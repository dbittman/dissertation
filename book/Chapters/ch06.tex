...
\squo{All problems in computer science can be solved by another level of indirection.}{David Wheeler}
\marginpar[]{\emph{...except the problem of too much indirection.}}
...
\begin{equation*}
    ...
    \right)
\end{equation*}
\annotate[yshift=0.5em]{above}{t}{Target object ID}
\annotate[yshift=-0.2em]{below}{fl}{Flags for this FOT entry}
...
\end{equation}
\annotate[yshift=0.5em]{above}{f}{Index into the FOT}
\annotate[yshift=0.5em]{above,left}{fot}{FOT Index Operation}
\annotate[yshift=-1.5em]{below}{t}{Target object ID}
\annotatetwo[yshift=-0.2em]{below}{n}{n2}{An offset into an object}
\vspace{0.4em}
...
%Thus the overall space trade off is, for \Twizzler, no additional space overhead
...
in-object string table that contains a name\sidenote{The name pointer is packed into the FOT entry by replacing the
    target object ID and setting a bit in the flags field to indicate that this entry contains a name.}.
...
means we can avoid increasing pointer size\sidenote{As we will see, this choice is meaningful for
    performance overhead.}, we must  consider the size of that table when
...
Fat pointers are present in, for example, PMDK~\cite{pmdk-pointers}, which includes an address space of
...
\annotate[yshift=0.5em]{above,left}{fot}{Total \# of FOT entries}
...
\begin{SCfigure}[t]
    ...
    \caption[Latency of KVS operations]{Latency of insert and lookup in \nvkv and \unixkv.
        An ``(m)'' indicates
