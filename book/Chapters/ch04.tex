...
\squo{Clearly, for
    computer systems to be interesting, both input and output are required.}{Operating Systems:\\Three Easy
    Pieces~\cite{ostep}}
...
due to their latency\sidenote{And implicit kernel-provided persistence without application control can result in hazards~\cite{crotty22-mmap}.}. The other systems follow different strategies for implementing implicit
...
\begin{itemize}
    ...
    \item The \emph{Multikernel} approach~\cite{baumann:sosp09} considers each NUMA domain or each individual core as
          ...
          system instance, more like LegoOS~\cite{legos}, which we take inspiration from\sidenote{Though we focus more
              towards constructing data that can be shared and positioned within a global address space.}.
          Finally, multikernels do not address making hardware see a uniform
