...
\end{chabstract}
\unedit{
    We cannot use virtual addresses, so we need a new way to name a word of
    memory: an \emph{invariant pointer}. The invariant pointer encodes a more fundamental notion of data identity
    instead of an ephemeral address,
    allowing any thread or device to name the desired word of memory regardless of
    address space or node.
    This approach dramatically improves programmability, as programmers
    need not worry about the complexity of referring to shared data with ephemeral constructs,
